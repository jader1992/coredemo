...
```
[~/Documents/workspace/gocore_workspace]$ ./testdemo dev
dev mode
...
使用命令 `./testdemo dev frontend`

要求当前编译机器安装 npm 软件，并且当前项目已经运行了 npm install，安装完成前端依赖。

```
[~/Documents/workspace/gocore_workspace]$ ./testdemo dev frontend
...
使用命令 `./testdemo dev backend`

要求当前编译机器安装 go 软件，版本 > 1.3。

```
[~/Documents/workspace/gocore_workspace]$  ./testdemo dev backend
./testdemo dev backend
...
```
[~/Documents/workspace/gocore_workspace]$ ./testdemo dev all
