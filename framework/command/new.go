...
var newCommand = &cobra.Command{
	...
	RunE: func(c *cobra.Command, args []string) error {
...

		{
			...
			prompt := &survey.Input{
				Message: "请输入版本名称(参考 https://github.com/jader1992/gocore/releases，默认为最新版本)：",
			}
			...
			if version != "" {
				// 确认版本是否正确
				release, _, err := client.Repositories.GetReleaseByTag(context.Background(), "jader1992", "gocore", version)
				if err != nil || release == nil {
					fmt.Println("版本不存在，创建应用失败, 请参考：https://github.com/jader1992/gocore/releases")
					return nil
				}
			}

			if version == "" {
				release, _, err = client.Repositories.GetLatestRelease(context.Background(), "jader1992", "gocore")
		...
		fmt.Println("gocore框架版本：", release.GetTagName())

		// 创建临时目录
		templateFolder := filepath.Join(currentPath, "template-gocore-"+version+"-"+cast.ToString(time.Now().Unix()))
		_ = os.Mkdir(templateFolder, os.ModePerm)
		fmt.Println("创建临时目录", templateFolder)

		// 下载项目
		url := release.GetZipballURL()
		err := util.DownloadFile(filepath.Join(templateFolder, "template.zip"), url) // 下载压缩包
		if err != nil {
			return err
		}
		...
		for _, fInfo := range fInfos {
			if fInfo.IsDir() && strings.Contains(fInfo.Name(), "jader1992-gocore-") {
		...
		_ = filepath.Walk(folder, func(path string, info fs.FileInfo, err error) error {
			...
			if path == filepath.Join(folder, "go.mod") {
				...
				c = bytes.ReplaceAll(c, []byte("module github.com/jader1992/gocore"), []byte("module "+mod)) // 创建应用名
				c = bytes.ReplaceAll(c, []byte("require ("), []byte("require (\n\tgithub.com/jader1992/gocore "+version))
			...
			isContain := bytes.Contains(c, []byte("github.com/jader1992/gocore/app"))
			if isContain {
				fmt.Println("更新文件:", path)
				c = bytes.ReplaceAll(c, []byte("github.com/jader1992/gocore/app"), []byte(mod+"/app"))
				err = ioutil.WriteFile(path, c, 0644)
				if err != nil {
					return err
				}
			...
			isContainCmd := bytes.Contains(c, []byte("./gocore"))
			if isContainCmd {
				fmt.Println("更新文件:", path)
				c = bytes.ReplaceAll(c, []byte("./gocore"), []byte("./"+name))
				err = ioutil.WriteFile(path, c, 0644)
				if err != nil {
					return err
				}
