...
func (p *Proxy) newProxyReverseProxy(frontend, backend *url.URL) *httputil.ReverseProxy {
	...
	// 两个都有: 先创建一个后端服务的directory
	director := func(req *http.Request) {
		if req.URL.Path == "/" || req.URL.Path == "/app.js" { // 如果路径是这两个直接转发到前端路由
...
func (p *Proxy) rebuildBackend() error {
	...
	// start 启动指定的命令但不等待它完成, 如果 Start 成功返回，则 c.Process 字段将被设置
	if err := cmdBuild.Start(); err == nil {
		// 等待命令退出并等待任何复制到 stdin 或从 stdout 或 stderr 复制完成
		err := cmdBuild.Wait()
		if err != nil {
			return err
		}
...
// startProxy 开启网关，且根据参数启动前端服务或者后端服务
